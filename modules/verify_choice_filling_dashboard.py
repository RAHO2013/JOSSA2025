    ...
    try:
        ...
        with tab1:
            ...
            desired_display_columns = [
                ...
                'PROGRAM', # From Master (normalized name + suffix, as 'Program' likely collides)
            ...
            cutoff_display_columns = [
                'OC_FEM', 'OC_GEN', 'EWS_FEM', 'EWS_GEN', 'OBC_FEM', 'OBC_GEN',
                'SC_FEM', 'SC_GEN', 'ST_FEM', 'ST_GEN'
            ]
            format_dict = {}
            for col in cutoff_display_columns:
        ...
        with tab2:
            # STUDENT ORDER RANGES:
            # ADJUST 'PROGRAM_master' to 'PROGRAM' if the debug output shows 'PROGRAM' without suffix
        ...
        with tab4:
            # This function's definition is included below, addressing "display_validation not defined"
            display_validation(merged_df, master_df, student_df)

        with tab5:
            st.subheader("📊 Visual Overview")
            col1, col2 = st.columns(2) # You have two columns here, but only one chart will use col1 explicitly

            with col1: # This column will contain Program Type Distribution
                # DASHBOARD CHARTS:
                # ADJUST THIS NAME BASED ON YOUR DEBUG OUTPUT IF IT DOESN'T APPEAR!
            ...
            # The Statewise Options Filled chart has been removed as per your last request.
            # The third chart (Institute Type) will now likely take up the remaining width.
...
def display_student_order_ranges(df):
    ...
    # You might need to change 'PROGRAM_master' to 'PROGRAM' if the debug output shows 'PROGRAM'
...
def display_dashboard_charts(df):
    ...
    col1, col2 = st.columns(2) # Keep 2 columns for potential future use or balanced layout
    ...
    # Statewise Options Filled chart is removed as per your request.
